...
# ===================== 1. 加载 AD vs LMCI 数据 =====================
print("🔹 正在加载 adni_ad_lmci.mat ...")

data = scio.loadmat("adni_ad_lmci.mat")
...
# 拼接数据
X_mri = np.vstack([ad_mri, lmci_mri])
X_pet = np.vstack([ad_pet, lmci_pet])
X_csf = np.vstack([ad_csf, lmci_csf])
X = np.hstack([X_mri, X_pet, X_csf])
y = np.hstack([ad_gnd, lmci_gnd])
...
df.to_excel("baseline_ad_lmci_result.xlsx", index=False)

print("\n" + "="*60)
print("🎉 AD vs LMCI 基线全部完成！")
print("✅ 结果已保存：baseline_ad_lmci_result.xlsx")